...
const ChoiceReadingPart6 = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await axios.get(`${apiUrl}/getAllReadingPart6`);
  ...
  if (questionGroups.length === 0) return <p className="text-white">Loading...</p>;

  return (
    <div className="p-4">
      <h2 className="text-2xl font-bold text-black mb-4">แบบทดสอบ (Reading Part 6 )</h2>
        ...
        <div key={group.id} className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {group.questions.map((question) => {
              ...
              return (
                <div key={questionId} className="p-4 bg-white shadow-md rounded-lg flex flex-col">
                  <p className="pb-3 text-lg font-semibold">{question.QuestionText}</p>
      ...
      {isSubmitted && (
        <p className="text-xl font-bold mt-6 text-black">
          ...
          {questionGroups.reduce((sum, group) => sum + group.questions.length, 0)})
          {score >= 80 ? (
...
export default ChoiceReadingPart6;
