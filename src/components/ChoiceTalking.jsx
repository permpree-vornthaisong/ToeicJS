...
const ChoiceConver = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${apiUrl}/getAllTalking`);
  ...
  const handleSubmit = () => {
    ...
    groups.forEach((group) => {
      for (let i = 1; group[`Question${i}`]; i++) {
        ...
        const correctAnswer = group[`choice${i}_${correctAnswerKey.slice(-1)}`];
